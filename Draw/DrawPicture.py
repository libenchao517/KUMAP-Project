...
class Draw_Pairhist:
    ...
    def Draw_pairhist(self, dist_train, dist_test):
        ...
        hist_down, bins_down = np.histogram(dist_test, bins=self.bar_num, range=(self.range_min, self.range_max))
...
class DrawBars:
    ...
    def __init__(
        ...
        self.xlabel = xlabel
        self.ylabel = ylabel
        self.color = np.random.random((100, 3)) if color is None else color
        self.legend_flag = legend_flag
        self.axis_flag = axis_flag
        self.axis_expty = axis_expty
        self.formats = formats
        self.dpi = dpi
        os.makedirs(self.path, exist_ok=True)

    def grouped_bars(self, values, groups=None, ylim=None, figsize=(6, 16)):
        """
        绘制垂直柱状图
        :param values: 柱的高度 [字典]
        :param groups: 刻度列表
        :param ylim:
        :param figsize:
        :return:
        """
        x = np.arange(len(values.get(list(values.keys())[0])))
        width = 0.8 / len(values)
        fig, ax = plt.subplots(figsize=figsize, layout='constrained')
        for i, (attribute, measurement) in enumerate(values.items()):
            ax.bar(x + width * i, measurement, width, label=attribute, color=self.color[i], edgecolor='#000000')
        if ylim is not None:
            ax.set_ylim(*ylim)
        ax.set_ylabel(self.ylabel, fontsize=self.titlefontsize)
        if not self.axis_expty:  # 是否设置空刻度
            ax.set_xticks(x- width/2 + 0.4, groups, fontsize=self.fontsize)
            plt.yticks(fontsize=self.fontsize)
        else:
            ax.set_xticklabels([])
            ax.set_yticklabels([])

        if self.legend_flag:   # 是否设置图例
            ax.legend(loc='best', fontsize = self.fontsize)
        if not self.axis_flag: # 是否关闭坐标轴
            plt.axis('off')
        Save_Pictures(os.path.join(self.path, self.filename), formats=self.formats, dpi=self.dpi)

    def grouped_barhs(self, values, groups, ylim=None, figsize=(6, 16)):
        """
        绘制水平形柱状图
        :param values: 柱的长度 [字典]
        :param groups: 刻度列表
        :param ylim:
        :param figsize:
        :return:
        ...
        fig, ax = plt.subplots(figsize=figsize, layout='constrained')
        for i, (attribute, measurement) in enumerate(values.items()):
            ax.barh(x + width * i, measurement, width, label=attribute, color=self.color[i], edgecolor='#000000')
        if ylim is not None:
            ax.set_xlim(*ylim)
        ax.set_xlabel(self.ylabel, fontsize=self.titlefontsize)

        if not self.axis_expty:  # 是否设置空刻度
            ax.set_yticks(x- width/2 + 0.4, groups, fontsize=self.fontsize, rotation=90)
            plt.xticks(fontsize=self.fontsize)
        else:
            ax.set_xticklabels([])
            ax.set_yticklabels([])
