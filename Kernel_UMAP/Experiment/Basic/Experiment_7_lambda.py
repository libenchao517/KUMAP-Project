...
config = KUMAP_config()
paraname = "ita"
ita = [0.01] + [0.01 * (i+1) for i in range(4, 99, 5)]
################################################################################
# 定义模型
...
# 生产对象
    model.Product_Kernel_Object(
        ...
        label_num=2,
        ita=None)
    AKSS = Analysis_Kernel_Paras(object=model.Kernel_Object, para=ita, paraname=paraname)
################################################################################
# 运行实验
    AKSS.Analysis()
################################################################################
# 绘制结果
    ...
    Draw_Line_Chart(
        ...
        xlabel="Proportion $\lambda$ of Data Information",
        # fontsize=18,
        # titlefontsize=20,
