...
config = KUMAP_config()
paraname = "label_num"
label_num = [i for i in range(2, 21)]
################################################################################
# 定义模型
...
# 生产对象
    model.Product_Kernel_Object(
        ...
        label_num=None,
        ita=0.5)
    AKSS = Analysis_Kernel_Paras(object=model.Kernel_Object, para=label_num, paraname=paraname)
################################################################################
# 运行实验
    AKSS.Analysis()
################################################################################
# 绘制结果
    ...
    Draw_Line_Chart(
        ...
        xlabel="Dimensions $\eta$ of Label Vectors",
        # fontsize=18,
        # titlefontsize=20,
